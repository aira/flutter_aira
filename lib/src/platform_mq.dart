...
abstract class PlatformMQ {
  ...
  Future<void> subscribe(
      String topic, MqttQos qosLevel, MessageCallback onMessage);
...
class PlatformMQImpl implements PlatformMQ {
  ...
  Future<void> _init(PlatformEnvironment env,
      {String? lastWillMessage, String? lastWillTopic}) async {
    ...
    await _client.connect('Token-${_session.token}',
        'x'); // The password can be any non-empty string
  ...
  static Future<PlatformMQ> create(
    ...
    await instance._init(env,
        lastWillMessage: lastWillMessage, lastWillTopic: lastWillTopic);
    return instance;
  }
  ...
  @override
  Future<void> subscribe(
      String topic, MqttQos qosLevel, MessageCallback onMessage) async {
